...
namespace ProgressBar {
    ...
    public class ProgressBarBehaviour : MonoBehaviour, IIncrementable, IDecrementable {
        ...
        public FillerProperty FillerInfo {
            get {
                if (m_FillerInfo == null && initialized) {
        ...
        void OnEnable () {
            InvokeRepeating ("TryInit", 0, 0.2f);
        }

        void TryInit () {
            if (m_FillRect.rect.width > 0) {
                initialized = true;
        ...
        void Init () {
            ...
            SetFillerSize (FillerInfo.MaxWidth);
            //We initialize m_Value
            m_Value = new ProgressValue (FillerInfo.MaxWidth, FillerInfo.MaxWidth);
            TransitoryValue = FillerInfo.MaxWidth;
        }
