  ...
  public class NetworkTransform_CA : NetworkBehaviour
  {
    ...
    private void SerializeMode3D(NetworkWriter writer)
    {
      if (this.isServer && (double) this.m_LastClientSyncTime != 0.0)
      {
        ...
        NetworkTransform_CA.SerializeVelocity3D(writer, this.m_TargetSyncVelocity, NetworkTransform_CA.CompressionSyncMode.None);
      ...
      else
      {
        ...
        NetworkTransform_CA.SerializeVelocity3D(writer, this.m_RigidBody3D.velocity, NetworkTransform_CA.CompressionSyncMode.None);
        if (this.syncRotationAxis != NetworkTransform_CA.AxisSyncMode.None)
          NetworkTransform_CA.SerializeRotation3D(writer, this.m_RigidBody3D.rotation, this.syncRotationAxis, this.rotationSyncCompression);
      }
      if (this.m_SyncSpin)
        NetworkTransform_CA.SerializeSpin3D(writer, this.m_RigidBody3D.angularVelocity, this.syncRotationAxis, this.rotationSyncCompression);
