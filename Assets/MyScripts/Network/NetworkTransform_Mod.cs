...
namespace UnityEngine.Networking
{
  ...
  [AddComponentMenu("Network/NetworkTransformMod")]
  public class NetworkTransform_Mod : NetworkBehaviour
  {
    [SerializeField]
    private float m_SendInterval = 0.1f;
    [SerializeField]
    private NetworkTransform_Mod.AxisSyncMode m_SyncRotationAxis = NetworkTransform_Mod.AxisSyncMode.AxisXYZ;
    [SerializeField]
    ...
    [SerializeField]
    private NetworkTransform_Mod.TransformSyncMode m_TransformSyncMode;
    [SerializeField]
    private NetworkTransform_Mod.CompressionSyncMode m_RotationSyncCompression;
    [SerializeField]
    private bool m_SyncSpin;
    [SerializeField]
    private NetworkTransform_Mod.ClientMoveCallback3D m_ClientMoveCallback3D;
    [SerializeField]
    private NetworkTransform_Mod.ClientMoveCallback2D m_ClientMoveCallback2D;
    ...
    /// </summary>
    public NetworkTransform_Mod.TransformSyncMode transformSyncMode
    {
      get
      {
    ...
    /// </summary>
    public NetworkTransform_Mod.AxisSyncMode syncRotationAxis
    {
      get
      {
    ...
    /// </summary>
    public NetworkTransform_Mod.CompressionSyncMode rotationSyncCompression
    {
      get
      {
    ...
    /// </summary>
    public NetworkTransform_Mod.ClientMoveCallback3D clientMoveCallback3D
    {
      get
      {
    ...
    ///   <para>A callback that can be used to validate on the server, the movement of client authoritative objects.</para>
    /// </summary>
    public NetworkTransform_Mod.ClientMoveCallback2D clientMoveCallback2D
    {
      get
      {
    ...
    {
      if (this.m_TransformSyncMode < NetworkTransform_Mod.TransformSyncMode.SyncNone || this.m_TransformSyncMode > NetworkTransform_Mod.TransformSyncMode.SyncCharacterController)
        this.m_TransformSyncMode = NetworkTransform_Mod.TransformSyncMode.SyncTransform;
      if ((double) this.m_SendInterval < 0.0)
        this.m_SendInterval = 0.0f;
      if (this.m_SyncRotationAxis < NetworkTransform_Mod.AxisSyncMode.None || this.m_SyncRotationAxis > NetworkTransform_Mod.AxisSyncMode.AxisXYZ)
        this.m_SyncRotationAxis = NetworkTransform_Mod.AxisSyncMode.None;
    ...
    public override bool OnSerialize(NetworkWriter writer, bool initialState)
    {
      ...
      {
        case NetworkTransform_Mod.TransformSyncMode.SyncNone:
          return false;
        case NetworkTransform_Mod.TransformSyncMode.SyncTransform:
          this.SerializeModeTransform(writer);
          break;
        case NetworkTransform_Mod.TransformSyncMode.SyncRigidbody2D:
          this.SerializeMode2D(writer);
          break;
        case NetworkTransform_Mod.TransformSyncMode.SyncRigidbody3D:
          this.SerializeMode3D(writer);
          break;
        case NetworkTransform_Mod.TransformSyncMode.SyncCharacterController:
    ...
    private void SerializeModeTransform(NetworkWriter writer)
    {
      ...
      if (this.m_SyncRotationAxis != NetworkTransform_Mod.AxisSyncMode.None)
        NetworkTransform_Mod.SerializeRotation3D(writer, this.transform.rotation, this.syncRotationAxis, this.rotationSyncCompression);
      this.m_PrevPosition = this.transform.position;
      this.m_PrevRotation = this.transform.rotation;
      this.m_PrevVelocity = 0.0f;
    }
    ...
    private void SerializeMode3D(NetworkWriter writer)
    {
      if (this.isServer && (double) this.m_LastClientSyncTime != 0.0)
      {
        ...
        NetworkTransform_Mod.SerializeVelocity3D(writer, this.m_TargetSyncVelocity, NetworkTransform_Mod.CompressionSyncMode.None);
        if (this.syncRotationAxis != NetworkTransform_Mod.AxisSyncMode.None)
          NetworkTransform_Mod.SerializeRotation3D(writer, this.m_TargetSyncRotation3D, this.syncRotationAxis, this.rotationSyncCompression);
      }
      else
      {
        writer.Write(this.m_RigidBody3D.position);
        NetworkTransform_Mod.SerializeVelocity3D(writer, this.m_RigidBody3D.velocity, NetworkTransform_Mod.CompressionSyncMode.None);
        if (this.syncRotationAxis != NetworkTransform_Mod.AxisSyncMode.None)
          NetworkTransform_Mod.SerializeRotation3D(writer, this.m_RigidBody3D.rotation, this.syncRotationAxis, this.rotationSyncCompression);
      }
      if (this.m_SyncSpin)
        NetworkTransform_Mod.SerializeSpin3D(writer, this.m_RigidBody3D.angularVelocity, this.syncRotationAxis, this.rotationSyncCompression);
    ...
    private void SerializeMode2D(NetworkWriter writer)
    {
      if (this.isServer && (double) this.m_LastClientSyncTime != 0.0)
      {
        ...
        NetworkTransform_Mod.SerializeVelocity2D(writer, (Vector2) this.m_TargetSyncVelocity, NetworkTransform_Mod.CompressionSyncMode.None);
        if (this.syncRotationAxis != NetworkTransform_Mod.AxisSyncMode.None)
        {
          float rot = this.m_TargetSyncRotation2D % 360f;
          if ((double) rot < 0.0)
            rot += 360f;
          NetworkTransform_Mod.SerializeRotation2D(writer, rot, this.rotationSyncCompression);
        }
      }
      else
      {
        writer.Write(this.m_RigidBody2D.position);
        NetworkTransform_Mod.SerializeVelocity2D(writer, this.m_RigidBody2D.velocity, NetworkTransform_Mod.CompressionSyncMode.None);
        if (this.syncRotationAxis != NetworkTransform_Mod.AxisSyncMode.None)
        {
      ...
      if (this.m_SyncSpin)
        NetworkTransform_Mod.SerializeSpin2D(writer, this.m_RigidBody2D.angularVelocity, this.rotationSyncCompression);
    ...
    private void UnserializeModeTransform(NetworkReader reader, bool initialState)
    {
      if (this.hasAuthority)
      {
        ...
        if (this.syncRotationAxis == NetworkTransform_Mod.AxisSyncMode.None)
          return;
        NetworkTransform_Mod.UnserializeRotation3D(reader, this.syncRotationAxis, this.rotationSyncCompression);
      }
      else if (this.isServer && this.m_ClientMoveCallback3D != null)
      {
        ...
        if (this.syncRotationAxis != NetworkTransform_Mod.AxisSyncMode.None)
          rotation = NetworkTransform_Mod.UnserializeRotation3D(reader, this.syncRotationAxis, this.rotationSyncCompression);
      ...
      else
      {
        ...
        if (this.syncRotationAxis == NetworkTransform_Mod.AxisSyncMode.None)
          return;
        this.transform.rotation = NetworkTransform_Mod.UnserializeRotation3D(reader, this.syncRotationAxis, this.rotationSyncCompression);
      }
    }
    ...
    private void UnserializeMode3D(NetworkReader reader, bool initialState)
    {
      if (this.hasAuthority)
      {
        ...
        if (!this.syncSpin)
          return;
        NetworkTransform_Mod.UnserializeSpin3D(reader, this.syncRotationAxis, this.rotationSyncCompression);
      }
      else
      {
        ...
        else
        {
          ...
          if (this.syncRotationAxis != NetworkTransform_Mod.AxisSyncMode.None)
            this.m_TargetSyncRotation3D = NetworkTransform_Mod.UnserializeRotation3D(reader, this.syncRotationAxis, this.rotationSyncCompression);
        }
        if (this.syncSpin)
          this.m_TargetSyncAngularVelocity3D = NetworkTransform_Mod.UnserializeSpin3D(reader, this.syncRotationAxis, this.rotationSyncCompression);
        ...
        else
        {
          ...
          if ((double) this.interpolateRotation == 0.0 && this.syncRotationAxis != NetworkTransform_Mod.AxisSyncMode.None)
          {
          ...
          if ((double) this.m_InterpolateMovement == 0.0)
            this.m_RigidBody3D.position = this.m_TargetSyncPosition;
          if (!initialState || this.syncRotationAxis == NetworkTransform_Mod.AxisSyncMode.None)
            return;
          this.m_RigidBody3D.rotation = this.m_TargetSyncRotation3D;
        }
      }
    ...
    private void UnserializeMode2D(NetworkReader reader, bool initialState)
    {
      if (this.hasAuthority)
      {
        ...
        if (this.syncRotationAxis != NetworkTransform_Mod.AxisSyncMode.None)
        {
          double num1 = (double) NetworkTransform_Mod.UnserializeRotation2D(reader, this.rotationSyncCompression);
        }
        if (!this.syncSpin)
          return;
        double num2 = (double) NetworkTransform_Mod.UnserializeSpin2D(reader, this.rotationSyncCompression);
      }
      else
      {
        ...
        if (this.isServer && this.m_ClientMoveCallback2D != null)
        {
          ...
          float rotation = 0.0f;
          if (this.syncRotationAxis != NetworkTransform_Mod.AxisSyncMode.None)
            rotation = NetworkTransform_Mod.UnserializeRotation2D(reader, this.rotationSyncCompression);
        ...
        else
        {
          ...
          if (this.syncRotationAxis != NetworkTransform_Mod.AxisSyncMode.None)
            this.m_TargetSyncRotation2D = NetworkTransform_Mod.UnserializeRotation2D(reader, this.rotationSyncCompression);
        }
        if (this.syncSpin)
          this.m_TargetSyncAngularVelocity2D = NetworkTransform_Mod.UnserializeSpin2D(reader, this.rotationSyncCompression);
        if (this.isServer && !this.isClient)
        {
    ...
    public static void HandleTransform(NetworkMessage netMsg)
    {
      ...
      {
        NetworkTransform_Mod component = localObject.GetComponent<NetworkTransform_Mod>();
    ...
    private static void WriteAngle(NetworkWriter writer, float angle, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      switch (compression)
      {
        case NetworkTransform_Mod.CompressionSyncMode.None:
          writer.Write(angle);
          break;
        case NetworkTransform_Mod.CompressionSyncMode.Low:
          writer.Write((short) angle);
          break;
        case NetworkTransform_Mod.CompressionSyncMode.High:
          writer.Write((short) angle);
          break;
      }
    }

    private static float ReadAngle(NetworkReader reader, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      switch (compression)
      {
        case NetworkTransform_Mod.CompressionSyncMode.None:
    ...
    public static void SerializeVelocity3D(NetworkWriter writer, Vector3 velocity, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      writer.Write(velocity);
    }

    public static void SerializeVelocity2D(NetworkWriter writer, Vector2 velocity, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      writer.Write(velocity);
    }

    public static void SerializeRotation3D(NetworkWriter writer, Quaternion rot, NetworkTransform_Mod.AxisSyncMode mode, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      switch (mode)
      {
        case NetworkTransform_Mod.AxisSyncMode.AxisX:
          NetworkTransform_Mod.WriteAngle(writer, rot.eulerAngles.x, compression);
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisY:
          NetworkTransform_Mod.WriteAngle(writer, rot.eulerAngles.y, compression);
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisZ:
          NetworkTransform_Mod.WriteAngle(writer, rot.eulerAngles.z, compression);
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisXY:
          NetworkTransform_Mod.WriteAngle(writer, rot.eulerAngles.x, compression);
          NetworkTransform_Mod.WriteAngle(writer, rot.eulerAngles.y, compression);
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisXZ:
          NetworkTransform_Mod.WriteAngle(writer, rot.eulerAngles.x, compression);
          NetworkTransform_Mod.WriteAngle(writer, rot.eulerAngles.z, compression);
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisYZ:
          NetworkTransform_Mod.WriteAngle(writer, rot.eulerAngles.y, compression);
          NetworkTransform_Mod.WriteAngle(writer, rot.eulerAngles.z, compression);
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisXYZ:
          NetworkTransform_Mod.WriteAngle(writer, rot.eulerAngles.x, compression);
          NetworkTransform_Mod.WriteAngle(writer, rot.eulerAngles.y, compression);
          NetworkTransform_Mod.WriteAngle(writer, rot.eulerAngles.z, compression);
          break;
    ...
    public static void SerializeRotation2D(NetworkWriter writer, float rot, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      NetworkTransform_Mod.WriteAngle(writer, rot, compression);
    }

    public static void SerializeSpin3D(NetworkWriter writer, Vector3 angularVelocity, NetworkTransform_Mod.AxisSyncMode mode, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      switch (mode)
      {
        case NetworkTransform_Mod.AxisSyncMode.AxisX:
          NetworkTransform_Mod.WriteAngle(writer, angularVelocity.x, compression);
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisY:
          NetworkTransform_Mod.WriteAngle(writer, angularVelocity.y, compression);
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisZ:
          NetworkTransform_Mod.WriteAngle(writer, angularVelocity.z, compression);
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisXY:
          NetworkTransform_Mod.WriteAngle(writer, angularVelocity.x, compression);
          NetworkTransform_Mod.WriteAngle(writer, angularVelocity.y, compression);
    ...
    public static void SerializeSpin2D(NetworkWriter writer, float angularVelocity, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      NetworkTransform_Mod.WriteAngle(writer, angularVelocity, compression);
    }

    public static Vector3 UnserializeVelocity3D(NetworkReader reader, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      return reader.ReadVector3();
    }

    public static Vector3 UnserializeVelocity2D(NetworkReader reader, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      return (Vector3) reader.ReadVector2();
    }

    public static Quaternion UnserializeRotation3D(NetworkReader reader, NetworkTransform_Mod.AxisSyncMode mode, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      ...
      {
        case NetworkTransform_Mod.AxisSyncMode.AxisX:
          zero.Set(NetworkTransform_Mod.ReadAngle(reader, compression), 0.0f, 0.0f);
          identity.eulerAngles = zero;
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisY:
          zero.Set(0.0f, NetworkTransform_Mod.ReadAngle(reader, compression), 0.0f);
          identity.eulerAngles = zero;
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisZ:
          zero.Set(0.0f, 0.0f, NetworkTransform_Mod.ReadAngle(reader, compression));
          identity.eulerAngles = zero;
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisXY:
          zero.Set(NetworkTransform_Mod.ReadAngle(reader, compression), NetworkTransform_Mod.ReadAngle(reader, compression), 0.0f);
          identity.eulerAngles = zero;
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisXZ:
          zero.Set(NetworkTransform_Mod.ReadAngle(reader, compression), 0.0f, NetworkTransform_Mod.ReadAngle(reader, compression));
          identity.eulerAngles = zero;
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisYZ:
          zero.Set(0.0f, NetworkTransform_Mod.ReadAngle(reader, compression), NetworkTransform_Mod.ReadAngle(reader, compression));
          identity.eulerAngles = zero;
          break;
        case NetworkTransform_Mod.AxisSyncMode.AxisXYZ:
          zero.Set(NetworkTransform_Mod.ReadAngle(reader, compression), NetworkTransform_Mod.ReadAngle(reader, compression), NetworkTransform_Mod.ReadAngle(reader, compression));
          identity.eulerAngles = zero;
          break;
      }
      return identity;
    ...
    public static float UnserializeRotation2D(NetworkReader reader, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      return NetworkTransform_Mod.ReadAngle(reader, compression);
    }

    public static Vector3 UnserializeSpin3D(NetworkReader reader, NetworkTransform_Mod.AxisSyncMode mode, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      Vector3 zero = Vector3.zero;
      switch (mode)
      {
    ...
    public static float UnserializeSpin2D(NetworkReader reader, NetworkTransform_Mod.CompressionSyncMode compression)
    {
      return NetworkTransform_Mod.ReadAngle(reader, compression);
    }
